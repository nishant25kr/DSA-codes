...
public:
    Node* removeDuplicates(Node* head)
    {
        ...
        while (temp != NULL && temp->next != NULL) // Ensure temp and temp->next are valid
        {
            if (temp->data == temp->next->data) // Check for duplicate
            {
                Node* dup = temp->next;        // Store duplicate node
                temp->next = temp->next->next; // Skip the duplicate node
                
                if (temp->next != NULL)        // Update prev pointer if not NULL
                {
                    temp->next->prev = temp;
                }
